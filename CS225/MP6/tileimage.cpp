...
void TileImage::setPixel(int startX, int startY, int resolution){
  if(!Pixel.empty()){
    return;
  }
  if (getResolution() % resolution == 0) {
      ...
      for (int x = 0; x < resolution; x++) {
          for (int y = 0; y < resolution; y++) {
              ...
              HSLAPixel p = getScaledPixelInt(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
              Pixel.push_back(p);
          }
      }
      ...
      for (int x = 0; x < resolution; x++) {
          for (int y = 0; y < resolution; y++) {
              ...
              HSLAPixel p = getScaledPixelDouble(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
              Pixel.push_back(p);
          }
      }
  }
...
void TileImage::paste(PNG& canvas, int startX, int startY, int resolution) const {
      ...
      for (int x = 0; x < resolution; x++) {
          for (int y = 0; y < resolution; y++) {
              *(canvas.getPixel(startX + x, startY + y)) = Pixel[i];
              i++;
          }
      }
