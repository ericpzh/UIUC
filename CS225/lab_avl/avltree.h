...
class AVLTree
{
  ...
  private:
    ...
    Node* findNode(Node* root, Node* pre){
      if (pre->left){
        return Max(pre->left);
      }
      Node* ret = NULL;
    ...
    Node*& search(Node*& root,K key,K key2){
      if(root == NULL) return root;
      if(root -> left != NULL){
        if(root -> left -> key == key2) return root;
      }
      if(root -> right != NULL){
        if(root -> right -> key == key2) return root;
      }
      if(root -> key < key) return search(root->right,key,key2);
      return search(root->left,key,key2);
    }
